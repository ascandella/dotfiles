-- Copied from:
--  https://github.com/kabouzeid/dotfiles/blob/master/nvim/lua/lsp-documentcolors.lua
...
local function create_highlight(rgb_hex, options)
  ...
  if mode == "foreground" then
    ...
    vim.api.nvim_command(string.format("highlight %s guifg=%s guibg=#%s", highlight_name, fg_color,
                                       rgb_hex))
  end
...
function M.buf_attach(bufnr, options)
  ...
  local trigger_update_highlight, timer = require("ai/defer").debounce_trailing(
    M.update_highlight,
